...
def show_applications_screen(app):
    ...
    if not app.currently_logged_in_user:
        ...
        tk.Label(card, text="Please log in to manage application settings.", font=app.font_large, fg=frontend_config.TEXT_COLOR, bg=frontend_config.CARD_BG_COLOR, wraplength=400).pack(expand=True)
        return
    
    user = app.currently_logged_in_user
    card = ui_helpers.create_main_card(app, width=780, height=380)
    cards_frame = tk.Frame(card, bg=frontend_config.CARD_BG_COLOR)
    cards_frame.pack(expand=True)
    
    # Helper to create the cards
    def _create_app_card(parent):
        c = tk.Frame(parent, bg=frontend_config.CARD_BG_COLOR, width=220, height=280, relief="solid", bd=1)
    ...
    tk.Label(pw_card, image=app.key_img, bg=frontend_config.CARD_BG_COLOR).pack(pady=(20, 0))
    tk.Label(pw_card, text="Password", font=app.font_medium_bold, fg=frontend_config.TEXT_COLOR, bg=frontend_config.CARD_BG_COLOR).pack(pady=(10, 5))
    tk.Label(pw_card, text="********", font=app.font_large, fg=frontend_config.TEXT_COLOR, bg=frontend_config.CARD_BG_COLOR).pack(pady=5)
    tk.Button(pw_card, text="Edit Password", font=app.font_small, bg=frontend_config.BUTTON_LIGHT_COLOR, fg=frontend_config.BUTTON_LIGHT_TEXT_COLOR, relief="solid", bd=1, padx=10, command=lambda: messagebox.showinfo("Info", "Password management is a future feature.")).pack(side=tk.BOTTOM, pady=(10, 15))
    ...
    tk.Label(voice_card, image=app.mic_img, bg=frontend_config.CARD_BG_COLOR).pack(pady=(20, 0))
    tk.Label(voice_card, text="Voice Biometrics", font=app.font_medium_bold, fg=frontend_config.TEXT_COLOR, bg=frontend_config.CARD_BG_COLOR).pack(pady=(10, 5))
    tk.Label(voice_card, text=f"Status: {voice_status}", font=app.font_normal, fg=frontend_config.TEXT_COLOR, bg=frontend_config.CARD_BG_COLOR).pack(pady=5)
    tk.Button(voice_card, text="Edit Biometrics", font=app.font_small, bg=frontend_config.BUTTON_LIGHT_COLOR, fg=frontend_config.BUTTON_LIGHT_TEXT_COLOR, relief="solid", bd=1, padx=10, command=app.navigate_to_enrollment).pack(side=tk.BOTTOM, pady=(10, 15))
    
    # OTP Settings Card
    masked_email = app._mask_email(user.get('email', ''))
    otp_card = _create_app_card(cards_frame)
    tk.Label(otp_card, image=app.otp_img, bg=frontend_config.CARD_BG_COLOR).pack(pady=(20, 0))
    tk.Label(otp_card, text="OTP Settings", font=app.font_medium_bold, fg=frontend_config.TEXT_COLOR, bg=frontend_config.CARD_BG_COLOR).pack(pady=(10, 5))
    tk.Label(otp_card, text="Account:", font=app.font_normal, fg=frontend_config.TEXT_COLOR, bg=frontend_config.CARD_BG_COLOR).pack(pady=(5, 0))
    tk.Label(otp_card, text=masked_email, font=app.font_normal, fg=frontend_config.TEXT_COLOR, bg=frontend_config.CARD_BG_COLOR).pack(pady=(0, 5))
    tk.Button(otp_card, text="Edit Email Address", font=app.font_small, bg=frontend_config.BUTTON_LIGHT_COLOR, fg=frontend_config.BUTTON_LIGHT_TEXT_COLOR, relief="solid", bd=1, padx=10, command=lambda: messagebox.showinfo("Info", "Email management is a future feature.")).pack(side=tk.BOTTOM, pady=(10, 15))
...
def show_about_screen(app, event=None):
    ...
    title_font = font.Font(family=frontend_config.FONT_FAMILY, size=22, weight="bold")
    ...
    body_font = font.Font(family=frontend_config.FONT_FAMILY, size=12)
    ...
    subtitle_font = font.Font(family=frontend_config.FONT_FAMILY, size=16, weight="bold")
